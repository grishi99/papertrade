...
import { TrendingUp, Wallet, LayoutDashboard, List, History, Settings, Bot, ChevronUp, ChevronDown, Search, CheckCircle2, MessageSquare, Tag, FileText } from 'lucide-react'
...
function App() {
    ...
    useEffect(() => {
        const fetchPrice = async () => {
            const data = await marketData.getLatestPrice(selectedSymbol)
            setPriceInfo(data)
        }
        fetchPrice()
                    ...
                    <div className="flex items-center space-x-4">
                        <div className="relative">
                            ...
                            <input
                                ...
                                onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                        setSelectedSymbol((e.target as HTMLInputElement).value.toUpperCase())
                                    }
                                }}
                            />
                        </div>
                            ...
                            <section className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex bg-secondary/50 p-1 rounded-xl border border-border/50">
                                        {['1m', '5m', '15m', '1h', '1D', '1W'].map(tf => (
                                            <button key={tf} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${tf === '1D' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-neutral-500 hover:text-white'}`}>
                                                {tf}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    <div className="lg:col-span-2 bg-card/30 p-8 rounded-3xl border border-border/50">
                                        <h3 className="text-sm font-bold uppercase tracking-widest text-neutral-500 mb-6">Market Statistics</h3>
                                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                                            <div>
                                                <div className="text-[10px] text-neutral-500 uppercase font-black mb-1">Open</div>
                                                <div className="font-mono text-lg font-bold">₹2,920.00</div>
                                            </div>
                                            <div>
                                                <div className="text-[10px] text-neutral-500 uppercase font-black mb-1 text-green-500/50">Day High</div>
                                                <div className="font-mono text-lg font-bold text-green-500 text-glow-green">₹2,975.30</div>
                                            </div>
                                            <div>
                                                <div className="text-[10px] text-neutral-500 uppercase font-black mb-1 text-red-500/50">Day Low</div>
                                                <div className="font-mono text-lg font-bold text-red-500 text-glow-red">₹2,910.15</div>
                                            </div>
                                            <div>
                                                <div className="text-[10px] text-neutral-500 uppercase font-black mb-1">Volume</div>
                                                <div className="font-mono text-lg font-bold">14.2M</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-primary/5 p-8 rounded-3xl border border-primary/20 relative overflow-hidden group cursor-pointer hover:bg-primary/10 transition-colors" onClick={() => setActiveTab('ai')}>
                                        ...
                                        <div className="flex items-start space-x-4 relative z-10">
                                            ...
                                            <div>
                                                <h4 className="text-lg font-bold text-primary mb-2">AI Trading Insights</h4>
                                                <p className="text-sm text-neutral-400 leading-relaxed font-medium">
                                                    "Prices are testing the <strong>₹2,940 resistance</strong>. Look for a break above with volume for a momentum play."
                                                </p>
                            ...
                            <aside className="w-96 border-l border-border flex flex-col bg-card/10 backdrop-blur-3xl shrink-0">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div>
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                <span className="text-[10px] text-primary font-bold">Max: {Math.floor(balance / (priceInfo?.price || 1))}</span>
                                            </div>
                                        ...
                                        <div className="pt-6 border-t border-border/50 space-y-3">
                                            <div className="flex justify-between text-[11px] font-bold">
                                                <span className="text-neutral-500 uppercase tracking-tighter">Value</span>
                                                <span className="font-mono text-white">₹{((priceInfo?.price || 0) * quantity).toLocaleString()}</span>
                                            </div>
                                            <button
                                                onClick={handlePlaceOrder}
                                                className={`w-full py-5 rounded-2xl font-black text-xs uppercase tracking-[0.3em] transition-all transform active:scale-95 shadow-2xl ${side === 'buy' ? 'bg-green-500 hover:bg-green-600 shadow-green-500/20' : 'bg-red-500 hover:bg-red-600 shadow-red-500/20'}`}
                                            >
                                                Place {side} order
                                            </button>
                                        </div>
                                ...
                                <div className="flex-1 flex flex-col overflow-hidden">
                                    <div className="p-6 border-b border-border/50 flex justify-between items-center bg-card/20">
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500">Market Watch</span>
                                    </div>
                                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                                        {[
                                            ...
                                            { s: 'NIFTY_BANK', p: '52,430.00', c: '+0.9%', up: true },
                                        ].map((item) => (
                                            <button
                                                key={item.s}
                                            ...
                                            >
                                                ...
                                                <div className="text-right">
                                                    <div className="font-mono text-sm font-black text-glow-white">₹{item.p}</div>
                                                    <div className={`text-[10px] font-black rounded px-1.5 py-0.5 mt-1 inline-block ${item.up ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                                                        {item.c}
                                                    </div>
                                                </div>
                                            </button>
                        ...
                        <section className="flex-1 overflow-y-auto p-12 space-y-12 custom-scrollbar">
                            <div className="flex items-end justify-between">
                                <div>
                                    ...
                                    <p className="text-neutral-500 mt-2 font-medium">Real-time breakdown of your virtual assets.</p>
                                </div>
                                <div className="bg-primary/10 border border-primary/20 px-6 py-3 rounded-2xl flex items-center space-x-3">
                                    ...
                                    <span className="text-sm font-bold text-white">Active Growth Portfolio</span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                                {[
                                    ...
                                    { label: 'Unrealised P&L', value: `${totalPnL >= 0 ? '+' : ''}₹${totalPnL.toLocaleString()}`, icon: ChevronUp, color: totalPnL >= 0 ? 'text-green-500' : 'text-red-500' },
                                    { label: 'Win Rate', value: '72%', icon: CheckCircle2, color: 'text-purple-500' },
                                ].map((stat, i) => (
                                            ...
                                            return (
                                                <tr key={pos.symbol} className="hover:bg-primary/5 transition-all group">
                                                    <td className="px-8 py-7">
                                                        ...
                                                        <div className="text-[10px] font-bold text-neutral-500 uppercase mt-1">Equity · Long</div>
                                                    </td>
                                                    ...
                                                    <td className="px-8 py-7 font-mono text-right font-bold transition-all group-hover:text-glow-white">₹{pos.currentPrice.toFixed(2)}</td>
                                            ...
                                            <tr>
                                                <td colSpan={6} className="px-8 py-32 text-center">
                                                    <div className="flex flex-col items-center space-y-4 opacity-30">
                                                        ...
                                                        <button onClick={() => setActiveTab('dashboard')} className="text-[10px] font-black text-primary border-b border-primary hover:text-white hover:border-white transition-colors">Start Trading Now</button>
                                                    </div>
                                                </td>
                                            </tr>
                                        )}
                        ...
                        <section className="flex-1 overflow-y-auto p-12 space-y-12 custom-scrollbar">
                            <div className="flex items-end justify-between">
                                <div>
                                    ...
                                    <p className="text-neutral-500 mt-2 font-medium">Capture your psychology and strategy for every trade.</p>
                                </div>
                                ...
                                {trades.length > 0 ? trades.slice().reverse().map((trade) => (
                                    <div key={trade.id} className="bg-card/30 rounded-3xl border border-border/50 p-8 flex flex-col md:flex-row md:items-center justify-between gap-8 hover:bg-secondary/20 transition-all border-l-8 border-l-primary">
                                        ...
                                        <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-8 px-8 border-x border-border/30">
                                            ...
                                            <div className="flex flex-col items-start">
                                                <div className="text-[10px] text-neutral-500 uppercase font-black mb-1">Strategy Tag</div>
                                                <div className="bg-secondary px-3 py-1 rounded-full text-[10px] font-black text-white hover:bg-primary transition-colors cursor-pointer flex items-center space-x-1.5">
                                                    <Tag size={10} />
                                                    <span>BREAKOUT</span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="w-full md:w-64 flex items-center space-x-4 bg-background/50 p-4 rounded-2xl border border-border/50">
                                            ...
                                            <textarea
                                                ...
                                                className="bg-transparent border-none focus:ring-0 w-full text-xs font-medium resize-none placeholder:text-neutral-700"
                                                rows={1}
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-card/20 rounded-[3rem] p-32 text-center border border-dashed border-border/50">
                                        ...
                                        <h3 className="text-xl font-black uppercase tracking-widest opacity-30">No Trade History</h3>
                                    </div>
                                )}
                            </div>
                        </section>
                        ...
                        <section className="flex-1 p-12 overflow-hidden flex flex-col space-y-8">
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-neutral-500 font-medium">Your personal companion for mastering the financial markets.</p>
                                </div>
                            </div>
                        ...
                        <section className="flex-1 p-12 overflow-y-auto space-y-12">
                            ...
                            <div className="max-w-2xl space-y-8">
                                <div className="bg-card/30 p-8 rounded-3xl border border-border/50 space-y-6">
                                    <h3 className="text-sm font-black uppercase tracking-widest text-neutral-500">Virtual Portfolio Configuration</h3>
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between p-4 bg-secondary/30 rounded-2xl">
                                            <div>
                                                <div className="text-sm font-bold">Commission Simulation</div>
                                                <div className="text-xs text-neutral-500">Apply virtual trading fees to trades.</div>
                                            </div>
                                            <div className="w-12 h-6 bg-primary rounded-full relative"><div className="absolute right-1 top-1 size-4 bg-white rounded-full" /></div>
                                        </div>
                                        <div className="flex items-center justify-between p-4 bg-secondary/30 rounded-2xl opacity-50">
                                            <div>
                                                <div className="text-sm font-bold">Slippage Simulation</div>
                                                <div className="text-xs text-neutral-500">Simulate price variations during execution.</div>
                                            </div>
                                            <div className="w-12 h-6 bg-neutral-700 rounded-full relative"><div className="absolute left-1 top-1 size-4 bg-neutral-400 rounded-full" /></div>
                                        </div>
                                    </div>
                                </div>

                                <div className="bg-card/30 p-8 rounded-3xl border border-border/50 space-y-6">
                                    <h3 className="text-sm font-black uppercase tracking-widest text-neutral-500">Account Reset</h3>
                                    <p className="text-xs text-neutral-500 leading-relaxed">Resetting your account will wipe all holdings, trade history, and restore your virtual balance to ₹10,00,000.</p>
                                    <button className="px-6 py-3 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all">Reset All Progress</button>
                                </div>
                            </div>
                        </section>
                    )}
